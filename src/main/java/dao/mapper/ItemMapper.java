...
public interface ItemMapper {
	...
	@Select("SELECT a.* "
			+ "    , ROUND(decode(cnt,0,0, score5/cnt) * 100, 1) AS score5_rate "
			+ "    , ROUND(decode(cnt,0,0, score4/cnt) * 100, 1) AS score4_rate "
			+ "    , ROUND(decode(cnt,0,0, score3/cnt) * 100, 1) AS score3_rate "
			+ "    , ROUND(decode(cnt,0,0, score2/cnt) * 100, 1) AS score2_rate "
			+ "    , ROUND(decode(cnt,0,0, score1/cnt) * 100, 1) AS score1_rate "
			+ "  FROM ( "
			...
			+ "	      , count(1) AS cnt "
			+ "	      , ROUND(nvl(avg(score),0),1) AS avg_score "
			+ "	  FROM REVIEW "
